...
function NavibarMobile() {
  ...
  return (
    <div>
      ...
      <BurgerMenu expanded={expanded} switcher={() => setExpanded(!expanded)} />
    </div>
  );
}
      ...
      {categories.map((category) => {
        return (
          <div
            ...
            className={location.pathname === `/cate/${category.path}` ? "activeMenuItem" : "inactiveMenuItem"}
          >
            {category.alias}
          </div>
        );
...
function CategoryBar() {
  ...
  return (
    <div className={flexRowCenter} style={{ alignItems: "center" }}>
      <CateItem path="/" title="首页" actived={location.pathname === "/"} />
      {categories.map((cate) => (
        <CateItem
          key={cate.path}
          path={`/cate/${cate.path}`}
