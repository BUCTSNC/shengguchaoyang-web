...
function GetCurrentWeather() {
    ...
    function judgewindlevel() {
        if (isNull(data.wind.speed) || isNull(data.wind.gust)) {
            return "--级";
        } else {
            if (
                beaufortScale(data.wind.speed) === beaufortScale(data.wind.gust)
            ) {
                return `${beaufortScale(data.wind.speed)}级`;
            } else {
                return `${beaufortScale(data.wind.speed)}-${beaufortScale(
                    data.wind.gust
                )}级`;
            }
        }
                ...
                <Spin spinning={isNull(data.cod)} size="large">
                    ...
                    <span className="PM25">
                        ...
                        {isNull(aqidata.list[0].components.pm2_5)
                            ? "--"
                            : aqidata.list[0].components.pm2_5.toFixed(0)}{" "}
